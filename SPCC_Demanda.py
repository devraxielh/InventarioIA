...
def EntrenarDemanda(BD, cantidad_particion, frecuencia, cp, cco, fecha):
    ...
    df = pd.DataFrame(
        columns=('centro_operativo', 'producto', 'cantidad_datos', 'mae_auto', 'mae_holt', 'mae_promedio'))
...
def VerficarDemanda(cco, cp, cantidad_particion, df, frecuencia, conn, fecha, BD):
    f = "where cod_centro_operativo=" + str(cco) + " and cod_producto=" + str(cp) + " And fecha_pedido<='" + (
        fecha) + "' order by fecha_pedido ASC"
...
def forecastProm(cp, ccp, c, BD, fecha):
    ...
    f = "where cod_centro_operativo=" + str(ccp) + " and cod_producto=" + str(
        cp) + " AND fecha_pedido BETWEEN date_add('" + fecha + "', INTERVAL -15 DAY) AND '" + fecha + "'"
    sql = "select round(avg(pedido_cantidad)) as pedido_cantidad from pedidos " + f + ""
    promedio = pd.read_sql(sql, conn)
